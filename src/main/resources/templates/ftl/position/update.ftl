...
<form modelAttribute="position" id="adminForm" method="post">
	...
	<thead>
	 <tr>
	  ...
	  <tr>
	    <td colspan="2" class="left">岗位修改-->[${position.name }]</td>
	  </tr>
	</thead>
	<tbody>
		<tr>
			<td class="right">岗位名称:</td>
			<td class="left"><input id="name" name="name" value="${position.name }"/><errors cssClass="errorContainer" path="name"/></td>
		</tr>
		<tr>
			<td class="right">岗位序号:</td>
			<td class="left"><input id="sn" name="sn" value="${position.sn }"/><errors cssClass="errorContainer" path="sn"/></td>
		</tr>
		<tr>
			<td colspan="2" class="center"><input type="submit" value="修改岗位"/></td>
		</tr>
	 </tbody>
	</table>
	</form>
